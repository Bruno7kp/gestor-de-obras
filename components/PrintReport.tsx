            ...
            {data.map((item) => {
              ...
              return (
                <tr key={item.id} className={isCat ? 'row-category' : 'row-item'}>
                  ...
                  <td className="text-center">{!isCat ? financial.formatQuantity(item.contractQuantity) : '-'}</td>
                  <td className="text-right">{financial.formatVisual(item.contractTotal, currencySymbol)}</td>
                  <td className="text-center">{!isCat ? financial.formatQuantity(item.previousQuantity) : '-'}</td>
                  <td className="text-right">{financial.formatVisual(item.previousTotal, currencySymbol)}</td>
                  <td className="text-center cell-medi-period">{!isCat ? financial.formatQuantity(item.currentQuantity || 0) : '-'}</td>
                  <td className="text-right cell-medi-period">{financial.formatVisual(item.currentTotal, currencySymbol)}</td>
                  <td className="text-center">{!isCat ? financial.formatQuantity(item.accumulatedQuantity) : '-'}</td>
                  <td className="text-right">{financial.formatVisual(item.accumulatedTotal, currencySymbol)}</td>
                  <td className="text-center">{!isCat ? financial.formatQuantity(item.balanceQuantity) : '-'}</td>
