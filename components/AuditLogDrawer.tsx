...
import { ChevronDown, ChevronRight, Clock, Filter, History, Loader2, Plus, Pencil, Trash2, X } from 'lucide-react';
...
function formatValue(val: unknown): string {
  ...
  if (typeof val === 'number') {
    // Try to detect monetary values (2 decimals)
...
const AuditCard: React.FC<{ entry: AuditLogEntry }> = ({ entry }) => {
  ...
  return (
    <article className="border border-slate-200 dark:border-slate-800 rounded-2xl p-4 transition-all hover:border-slate-300 dark:hover:border-slate-700">
...
/* ── Main drawer ──────────────────────────────────────────── */
export const AuditLogDrawer: React.FC<AuditLogDrawerProps> = ({ isOpen, onClose, projectId }) => {
  ...
  const fetchData = useCallback(
    async (pageNum: number, action?: ActionFilter, model?: string | null) => {
      setLoading(true);
      try {
        const params: Record<string, string | number> = { projectId, page: pageNum, pageSize: PAGE_SIZE };
  ...
  }, [isOpen, filter, modelFilter, fetchData]);
  ...
  // Unique models present for model filter chips
  ...
  return (
    <>
      ...
      {/* Panel */}
      <aside className="fixed top-0 right-0 h-screen w-full sm:w-[460px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 z-[150] shadow-2xl flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {loading && entries.length === 0 ? (
            <div className="flex items-center justify-center py-12 gap-2 text-xs text-slate-500 dark:text-slate-400">
              <Loader2 size={16} className="animate-spin" />
              Carregando histórico…
            </div>
          ) : entries.length === 0 ? (
            <div className="text-xs text-slate-500 dark:text-slate-400 text-center py-12">
            ...
            <>
              ...
              {hasMore && (
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50"
                >
                  {loading ? (
