  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        ...
        <div className={`p-8 rounded-[32px] border shadow-sm flex flex-col justify-between transition-all ${margin >= 0 ? 'bg-emerald-50/50 border-emerald-100 dark:bg-emerald-900/10 dark:border-emerald-800' : 'bg-rose-50/50 border-rose-100 dark:bg-rose-900/10 dark:border-rose-800'}`}>
          <div className="flex justify-between items-start">
            <div className={`p-3 rounded-xl ${margin >= 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
               <DollarSign size={20}/>
            </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex items-center gap-1 mt-1">
              {margin >= 0 ? <ArrowUpRight size={12} className="text-emerald-500"/> : <TrendingDown size={12} className="text-rose-500"/>}
              <span className={`text-[10px] font-bold uppercase ${margin >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{marginPercent.toFixed(1)}%</span>
            </div>
          </div>
        </div>
...
const KpiCard = ({ label, value, icon, progress, color }: any) => {
  const colors: any = { 
    indigo: 'text-indigo-600 bg-indigo-50 border-indigo-100 dark:bg-indigo-900/10 dark:border-indigo-800', 
    emerald: 'text-emerald-600 bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-800', 
    rose: 'text-rose-600 bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-800', 
    slate: 'text-slate-600 bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-800' 
  };
  return (
    <div className={`p-8 rounded-[32px] border shadow-sm hover:shadow-xl transition-all relative overflow-hidden group bg-white dark:bg-slate-900 dark:border-slate-800`}>
      <div className="flex justify-between items-start mb-6">
        <div className={`p-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-400 transition-colors`}>{icon}</div>
      ...
      <div className="w-full h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
        <div className={`h-full opacity-70 transition-all duration-1000 bg-indigo-500`} style={{ width: `${Math.min(100, Math.abs(progress || 0))}%` }} />
      </div>
    </div>
  );
};
