...
export const SupplierManager: React.FC<SupplierManagerProps> = ({ suppliers: homeSuppliers, projects, onUpdateSuppliers }) => {
  ...
  type SupplierViewMode = 'list' | 'table';
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {filteredSuppliers.map(supplier => (
                      <tr key={supplier.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0 ${getCategoryColor(supplier.category)}`}>
                              {supplier.category === 'Material' ? <Truck size={18} /> : <Building2 size={18} />}
                            </div>
                            <div>
                              <span className="text-xs font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight block">{supplier.name}</span>
                              {supplier.cnpj && (
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{supplier.cnpj}</span>
                              )}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase border ${
                            supplier.category === 'Material' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-800'
                            : supplier.category === 'Serviço' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-800'
                            : supplier.category === 'Locação' ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-800'
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1">
                            {supplier.contactName && (
                              <div className="text-[10px] font-bold text-slate-600 dark:text-slate-300">{supplier.contactName}</div>
                            )}
                            {supplier.email && (
                        ...
                        <td className="px-6 py-4">
                          {supplier.notes ? (
                            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 line-clamp-2">{supplier.notes}</span>
                          ) : (
                            <span className="text-[9px] text-slate-300 dark:text-slate-600 italic">—</span>
                          )}
                        </td>
