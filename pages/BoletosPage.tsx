...
import { AlertTriangle, CheckCircle2, Clock, Download, FileText, Filter, Plus, RefreshCw, Receipt, Save, X } from 'lucide-react';
...
import { ExpenseAttachmentZone } from '../components/ExpenseAttachmentZone';
...
export const BoletosPage: React.FC = () => {
  ...
  const handleCreate = async (projectId: string, payload: { description: string; amountDue: number; dueDate: string; attachmentUrl?: string | null }) => {
    await planningApi.createBoleto(projectId, payload);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {canEditPlanning && (
              <button onClick={() => setShowCreateModal(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:scale-105 transition-transform">
...
const CreateBoletoModal = ({
  ...
  onClose: () => void;
  onSave: (projectId: string, payload: { description: string; amountDue: number; dueDate: string; attachmentUrl?: string | null }) => Promise<void>;
}) => {
  const toast = useToast();
  ...
  const inputClass = "w-full px-4 py-3 rounded-2xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm font-bold outline-none focus:border-indigo-500 transition-all";

  const submit = async () => {
    if (!projectId) return toast.warning('Selecione uma obra.');
    ...
    if (!parsedAmount || parsedAmount <= 0) return toast.warning('Informe um valor válido para o boleto.');
    if (!dueDate) return toast.warning('Informe a data de vencimento.');
    setSaving(true);
    try {
      await onSave(projectId, { description: description.trim(), amountDue: parsedAmount, dueDate, attachmentUrl: attachmentUrl ?? null });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[2200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-8 md:p-10" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-black tracking-tight text-slate-800 dark:text-white">Novo Boleto</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            <input value={description} onChange={e => setDescription(e.target.value)} placeholder="Ex: Boleto fornecedor cimento" className={inputClass} />
          </div>

          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Valor a Pagar</label>
            <input value={amountDue} onChange={e => setAmountDue(financial.maskCurrency(e.target.value))} placeholder="0,00" className={inputClass} />
          </div>
        ...
        <div className="flex items-center justify-end gap-3 mt-8">
          ...
          <button onClick={() => void submit()} disabled={saving} className="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:scale-105 transition-transform disabled:opacity-50">
            <Save size={14} /> {saving ? 'Salvando...' : 'Cadastrar Boleto'}
          </button>
        </div>
      </div>
    </div>
