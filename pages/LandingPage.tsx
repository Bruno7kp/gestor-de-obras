  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="relative overflow-hidden">
        ...
        <header className="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-400">ProMeasure</p>
              <p className="text-lg font-semibold">Gestao Inteligente</p>
            </div>
          </div>
        ...
        <section className="relative z-10 mx-auto grid max-w-6xl gap-10 px-6 pb-16 pt-10 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-8">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-300">
              Centralize medicao, custos, contratos e compliance em uma unica base. Assine um plano e ative sua instancia em minutos.
            </p>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                { title: 'Fluxo de medicao', detail: 'Snapshots automaticos e auditoria completa.' },
                { title: 'EAP inteligente', detail: 'Calculo hierarquico com integridade total.' },
                { title: 'Gestao multi-instancia', detail: 'Controle centralizado para grupos.' },
              ].map((item) => (
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white/90 p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900/70">
            <div className="space-y-5">
              <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Visao geral</p>
              <h2 className="text-2xl font-display font-semibold">Seu cockpit de engenharia</h2>
              <p className="text-sm text-slate-500 dark:text-slate-300">
                Monitoramento em tempo real, com previsao financeira e alertas automatizados por etapa.
              </p>
              <div className="space-y-3">
                {["Custos vs orcamento", "Diario de obra inteligente", "Governanca por perfis"].map((line) => (
              ...
              <div className="rounded-3xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-700 dark:border-indigo-900/40 dark:bg-indigo-950/30 dark:text-indigo-200">
                Entrega imediata: configure sua instancia, importe seus dados e siga para a operacao.
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {["Essencial", "Profissional", "Enterprise"].map((plan, index) => (
            <div key={plan} className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">Precos e detalhes serao liberados no onboarding.</p>
              <Link
                to="/signup"
      ...
      <footer className="border-t border-slate-200 bg-white px-6 py-8 text-center text-xs text-slate-400 dark:border-slate-800 dark:bg-slate-950">
        ProMeasure Pro - Gestao de Engenharia SaaS
      </footer>
    </div>
  );
};
